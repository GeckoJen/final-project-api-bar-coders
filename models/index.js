...
export async function getCurrentBooks(id) {
      const data = await query(
            `SELECT * from allbooks FULL OUTER JOIN summaries on allbooks.id = summaries.book_id WHERE allbooks.student_id = $1 AND (summaries.iscomplete is NULL or summaries.iscomplete = false)`,
            [id]
      );
      return data.rows;
}
